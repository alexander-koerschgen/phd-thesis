...
\subsection{Spans of finite sets}
  ...
  \begin{defn}\label{def_bicategory_functor}
    Given two bicategieres $\cC$, $\cD$ a \emph{lax functor}, or \emph{lax
    2-functor}, $F = (F, \phi): \mathcal{C} \to \mathcal{D}$ consists of
    \begin{itemize}
      \item a map $F_0: \cC_0 \to \cD_0$ (denoted $F$);
      \item for each $a,b \in \cC_0$ a functor $F_{a,b}: \cC(a,b) \to \cD(F(a),
        F(b))$ (also denoted $F$),
      \item for each pair of composable 1-morphisms $(f,g)$ a 2-morphism
        $\phi_{g,f}:F(g)\circ F(f)\to F(g\circ f)$,
      \item for each $a \in \cC_0$, a 2-morphism $\phi_a: 1_{F(a)} \to F(1_a)$,
    \end{itemize}
    such that $\phi_{g,f}$ and $\phi_a$ are natural in $g,f$ and $a$,
    respectively, and the following diagrams commute for all composable
    morphisms $a \myrightarrow{f} b \myrightarrow{g} c \myrightarrow {h} d$:
    \begin{gather*}
  ...
  We are now going to define an abstraction of the Loday functor as it is
  ...
  \begin{defn}\label{def_J_and_functors_from_finite_sets_to_pointed_categories}
    ...
    \begin{displaymath}
      ...
      \left\{ %
       ...
       \right\}.
    \end{displaymath}
    % TODO might add remark here pointing out similarity to classic def
...
\subsection{Rectification}
  ...
  [LOL LUECKE!!!!!!!11111111111hundertelf]
  %
